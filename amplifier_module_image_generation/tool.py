...
class ImageGenerationTool:
    ...
    def input_schema(self) -> dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "operation": {
                    "type": "string",
                    "enum": ["generate", "check_availability", "get_cost_estimate"],
                    "description": "Operation to perform"
                },
                "prompt": {
                    "type": "string",
                    "description": "Image description (required for 'generate')"
                },
                "output_path": {
                    "type": "string",
                    "description": "Where to save the image (required for 'generate')"
                },
                "preferred_api": {
                    "type": "string",
                    "enum": ["openai", "google", "imagen", "dalle", "gptimage"],
                    "description": "Preferred provider (optional for 'generate')"
                },
                "params": {
                    "type": "object",
                    "description": "Provider-specific parameters (optional)"
                },
                "reference_image_path": {
                    "type": "string",
                    "description": "Single reference image path (optional for 'generate')"
                },
                "reference_image_paths": {
                    "type": "array",
                    "items": {"type": "string"},
                    "maxItems": 14,
                    "description": "Multiple reference images (optional for 'generate')"
                },
                "provider": {
                    "type": "string",
                    "description": "Provider name (required for 'check_availability' and 'get_cost_estimate')"
                }
            },
            "required": ["operation"]
        }
    ...
    async def execute(self, input: dict[str, Any]) -> ToolResult:
        ...
        Operations:
            generate:
                ...
                - reference_image_path (str, optional): Single reference image to guide generation
                - reference_image_paths (list[str], optional): Multiple reference images (up to 14)
    ...
    async def _execute_generate(self, input: dict[str, Any]) -> ToolResult:
        ...
        if not prompt:
            return ToolResult(
                success=False, error={"message": "Missing required field: 'prompt'"}
            )
        ...
        reference_image_path = input.get("reference_image_path")
        reference_image_paths = input.get("reference_image_paths")
    ...
    async def _execute_check_availability(self, input: dict[str, Any]) -> ToolResult:
        ...
        if not provider:
            return ToolResult(
                success=False, error={"message": "Missing required field: 'provider'"}
            )
